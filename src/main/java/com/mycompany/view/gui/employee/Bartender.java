...
public class Bartender extends javax.swing.JFrame {
    ...
    private void loadQuery() {
        ...
        al.add("bartender");
        al.add("employee,bartender");
        al.add("bartender_role,bartender");
    ...
    private void loadCombos() {
        LoadSubTypes.loadType(comboBox1, "bartender_role");
    ...
    private boolean empCheck() {
        ...
        ResultSet rs;
        try {
            rs = MySql.sq("SELECT * FROM `bartender` WHERE `employee_id`='" + empId + "'");
            if (rs.next()) {
                state = true;
            }
    ...
    private void addmanger() {
        if (comboBox1.getSelectedItem().toString().equals("Select bartender_role")) {
            Message m = new Message(this, "Select a valid bartender type", "warning");

        } else if (textF1.getText().isEmpty()) {
            Message m = new Message(this, "Fields are empty", "warning");
        } else {

            String bartenderType = comboBox1.getSelectedItem().toString();

            if (!empCheck()) {

                ResultSet rs;
                try {
                    rs = MySql.sq("SELECT * FROM `bartender_role` WHERE `bartender_role_name`='" + bartenderType + "'");
                    rs.next();
                    String bartenderTypeId = rs.getString("bartender_role_id");
                ...
                Message m = new Message(this, "This bartender Already exits ", "warning");
            }
        }
        ...
        if (comboBox1.getSelectedItem().toString().equals("Select bartender_role")) {
            ...
            if (empCheck()) {
                ...
                try {
                    ...
                    MySql.iud("UPDATE `bartender` SET `bartender_role_id`='" + bartenderTypeId + "' WHERE `employee_id`='" + empId + "'");
    ...
    private void initComponents() {
        ...
        jLabel5.setText("Bartender_Role");
        ...
        customTable1.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Emp_Id", "Bartender_Id", "Name", "Bartender_Type"
            }
        ));
